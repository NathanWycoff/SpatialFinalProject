...
##Description -- Comparison of methods to estimate a noise corrupted model with imperfect knowledge
## of the true model (parameter of diffeq is off).
## Attempt to incorperate dynamical system information in a GP prior.
...
for (ayy in 1:iters) {
  ...
  sample_df <- data.frame(x_samples * delta_x + min(dist_interval),
                          ...
                          sapply(1:n_samples, function(i) S[x_samples[i],t_samples[i]]),
                          sapply(1:n_samples, function(i) S[x_samples[i],t_samples[i]] + rnorm(1,0,sigma)))
  colnames(sample_df) <- c('x','t','Truth','Observed')
  ...
  #Actual Model Fitting
  precondition_fit <- spLM(Truth ~ 1, data = rbind(train,test), coords = rbind(train_coords,test_coords), starting = starting, priors = priors,
                       tuning = tuning, cov.model= 'gaussian', n.samples = 1000)
  ...
  results <- c(results, c(baseline_mae, dynamical_mae, standard_mae, conditioned_mae))
}

print(c(mean(results[seq(1,length(results), 4)]), mean(results[seq(2,length(results), 4)]),mean(results[seq(3,length(results), 4)]), mean(results[seq(4,length(results), 4)])))
